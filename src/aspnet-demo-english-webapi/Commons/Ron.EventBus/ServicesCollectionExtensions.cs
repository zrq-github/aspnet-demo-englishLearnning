...
public static class ServicesCollectionExtensions
{
    ...
    /// </summary>
    /// <remarks>
    /// 扫描 assemblies 中继承IIntegrationEventHandler的类
    /// </remarks>
    ...
    /// 如果多个消费者订阅同一个Queue，这时Queue中的消息会被平均分摊给多个消费者进行处理，而不是每个消费者都收到所有的消息并处理。为了确保一个应用监听到所有的领域事件，所以不同前端项目的queueName需要不一样。
    ...
    /// </param>
    /// <returns></returns>
    private static IServiceCollection AddEventBus(this IServiceCollection services, string queueName, IEnumerable<Type> eventHandlerTypes)
    {
        var handlerTypes = eventHandlerTypes as Type[] ?? eventHandlerTypes.ToArray();
        ...
        services.AddSingleton<IEventBus>(sp =>
        {
            ...
            var rabbitMQOptions = sp.GetRequiredService<IOptions<IntegrationEventRabbitMQOptions>>().Value;
            ...
            var eventBus = new RabbitMQEventBus(mqConnection, serviceScopeFactory, rabbitMQOptions.ExchangeName, queueName);
